...
> - After observing the effects of running our JavaScript code, **refresh the page**. Notice how those changes have disappeared? That's because we used JavaScript to change what was in *the browser*, not what was in our `index.html` file! Refreshing the page tells the browser to reload the `index.html`.
