...
1. Let's use a third-party CSS called [PicoCSS](https://picocss.com/). Add the following to the `<head>`, just under the `<title>`.
...
We observed that we can use the `.innerText` and `.innerHTML` properties allow us to retrieve the text and HTML inside a DOM element. We can also use these properties to *change* the content of DOM elements. It's time to discover how we can edit our document object model.
...
How might you build on this tutorial? Continue exploring the use of the `.querySelector()` function in the HTML page you built. Then go and explore how you can use it in the dev console for *any* web page - even those built by other people! Lastly, take a look at the [official documentation for the `.querySelector()` method on MDN](https://developer.mozilla.org/en-US/docs/Web/API/Document/querySelector).
