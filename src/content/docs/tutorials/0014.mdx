...
1. We can go a long way with just the basic math operators. Occasionally, however, we need to do complex math. One example is calculating a *square root*. Fortunately, there is a `Math` object in JavaScript that holds several useful functions for these situations.
    ...
    let message = `My triangle has sides of length ${adjacent}, ${opposite} and ${hypotenuse}.`;
    console.log(message);
    ```
...
1. An interesting aspect of converting strings to numbers is that these functions will omit trailing characters that are not digits.
    ...
    <Aside type="caution" title="Just because you can doesn't mean you should...">
      The `parseFloat()` and `parseInt()` functions are pretty forgiving when it comes to converting strings to numbers. As seen above, we *can* convert a string like `'3.14 is PI'` to the number `3.14`. But just because we *can* do that doesn't mean we *should* do that.

      If you are planning on extracting a number from a string that contains non-numeric characters, you're should be very careful about your expectations. If you're just at the start of learning JavScript, it's best to avoid this kind of thing until you have a better understanding of how these functions work and the potential pitfalls.

      But there is one takeaway that you should remember: Always be mindful of the data types you're working with and how functions handle them.
    </Aside>
...
<LinkButton href="https://dev.to/sarthak_niranjan_3bd6cb5f/javascript-round-to-2-decimal-places-a-complete-guide-323" target="_blank" icon="external">
