...
But what if we wanted some *variety* in how we communicate the ideas of *suits* and *values*? For example, we might want to talk about the "2 of Hearts" or the "Two of Hearts". One way uses the digit `2` while the other uses the word `"Two"`. Furthermore, we might interested in using *emojii symbols* for the suits.
